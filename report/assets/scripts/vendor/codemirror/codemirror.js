...
window.CodeMirror = (function () {
  ...
  const ie_lt8 = /MSIE [1-7]\b/.test(navigator.userAgent);
  const ie_lt9 = /MSIE [1-8]\b/.test(navigator.userAgent);
  ...
  const mac_geLion = /Mac OS X 1\d\D([7-9]|\d\d)\D/.test(navigator.userAgent);
  ...
  const mobile = ios || /Android|webOS|BlackBerry|Opera Mini|IEMobile/i.test(navigator.userAgent);
  ...
  function CodeMirror(place, options) {
    ...
    const display = this.display = makeDisplay(place);
    ...
    let hasFocus; try { hasFocus = (document.activeElement == display.input); } catch(e) { }
  ...
  function makeDisplay(place) {
    ...
    const scrollerInner = elt('div', [d.sizer, d.heightForcer, d.gutters], null, 'position: relative; min-height: 100%');
    // Provides scrolling
  ...
  function wrappingChanged(cm) {
    let doc = cm.view.doc, th = textHeight(cm.display);
    if (cm.options.lineWrapping) {
      cm.display.wrapper.className += ' CodeMirror-wrap';
      const perLine = cm.display.scroller.clientWidth / charWidth(cm.display) - 3;
    ...
    clearCaches(cm);
    setTimeout(() => {updateScrollbars(cm.display, cm.view.doc.height);}, 100);
  }

  function keyMapChanged(cm) {
    const style = keyMap[cm.options.keyMap].style;
  ...
  function computeMaxLength(view) {
    ...
    view.doc.iter(1, view.doc.size, (line) => {
      const len = lineLength(view.doc, line);
  ...
  function setGuttersForLineNumbers(options) {
    let found = false;
    for (let i = 0; i < options.gutters.length; ++i) {
  ...
  function updateScrollbars(d /* display */, docHeight) {
    const totalHeight = docHeight + 2 * paddingTop(d);
    d.sizer.style.minHeight = d.heightForcer.style.top = `${totalHeight }px`;
    const scrollHeight = Math.max(totalHeight, d.scroller.scrollHeight);
    const needsH = d.scroller.scrollWidth > d.scroller.clientWidth;
    const needsV = scrollHeight > d.scroller.clientHeight;
    if (needsV) {
  ...
  function visibleLines(display, doc, viewPort) {
    let top = display.scroller.scrollTop, height = display.wrapper.clientHeight;
  ...
  function alignHorizontally(cm) {
    ...
    const comp = compensateForHScroll(display) - display.scroller.scrollLeft + cm.view.scrollLeft;
    let gutterW = display.gutters.offsetWidth, l = `${comp }px`;
    for (let n = display.lineDiv.firstChild; n; n = n.nextSibling) {
      if (n.alignable) {
        for (let i = 0, a = n.alignable; i < a.length; ++i) a[i].style.left = l;
      }
    }
  ...
  function maybeUpdateLineNumberWidth(cm) {
    ...
    let doc = cm.view.doc, last = lineNumberFor(cm.options, doc.size - 1), display = cm.display;
  ...
  function updateDisplay(cm, changes, viewPort) {
    let oldFrom = cm.display.showingFrom, oldTo = cm.display.showingTo;
    const updated = updateDisplayInner(cm, changes, viewPort);
    if (updated) {
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    let display = cm.display, doc = cm.view.doc;
    ...
    const visible = visibleLines(display, doc, viewPort);
    ...
    let positionsChangedFrom = changes === true ? 0 : Infinity;
    ...
    let to = Math.min(doc.size, visible.to + cm.options.viewportMargin);
    ...
    intact.sort((a, b) => a.from - b.from);

    if (intactLines < (to - from) * 0.7) display.lineDiv.style.display = 'none';
    patchDisplay(cm, from, to, intact, positionsChangedFrom);
    display.lineDiv.style.display = '';

    const different = from != display.showingFrom || to != display.showingTo ||
    ...
    for (var node = display.lineDiv.firstChild, height; node; node = node.nextSibling) {
      if (node.lineObj) {
        if (ie_lt8) {
          const bot = node.offsetTop + node.offsetHeight;
        ...
        const diff = node.lineObj.height - height;
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    if (!intact.length && !ie && (!webkit || !cm.display.currentWheelTarget)) {removeChildren(display.lineDiv);}
    ...
    let nextIntact = intact.shift(), lineNo = from;
    cm.view.doc.iter(from, to, (line) => {
      ...
      if (lineIsHidden(line)) {
        ...
        const lineNode = buildLineElement(cm, line, lineNo, dims);
  ...
  function buildLineElement(cm, line, lineNo, dims) {
    const lineElement = lineContent(cm, line);
    let markers = line.gutterMarkers, display = cm.display;
    ...
    const wrap = elt('div', null, line.wrapClass, 'position: relative');
  ...
  function updateSelection(cm) {
    const display = cm.display;
    const collapsed = posEq(cm.view.sel.from, cm.view.sel.to);
    ...
    const headPos = cursorCoords(cm, cm.view.sel.head, 'div');
  ...
  function updateSelectionCursor(cm) {
    let display = cm.display, pos = cursorCoords(cm, cm.view.sel.head, 'div');
  ...
  function updateSelectionRange(cm) {
    let display = cm.display, doc = cm.view.doc, sel = cm.view.sel;
    const fragment = document.createDocumentFragment();
    let clientWidth = display.lineSpace.offsetWidth, pl = paddingLeft(cm.display);
    ...
    function drawForLine(line, fromArg, toArg, retTop) {
      ...
      let lineLen = lineObj.text.length, rVal = retTop ? Infinity : -Infinity;
      function coords(ch) {
        return charCoords(cm, { line, ch }, 'div', lineObj);
      }

      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, (from, to, dir) => {
        const leftPos = coords(dir == 'rtl' ? to - 1 : from);
        const rightPos = coords(dir == 'rtl' ? from : to - 1);
        let left = leftPos.left, right = rightPos.right;
    ...
    if (sel.from.line == sel.to.line) {
      ...
      const fromObj = getLine(doc, sel.from.line);
      let cur = fromObj, merged, path = [sel.from.line, sel.from.ch], singleLine;
      while (merged = collapsedSpanAtEnd(cur)) {
      ...
      if (singleLine) {
        for (let i = 0; i < path.length; i += 3) {drawForLine(path[i], path[i + 1], path[i + 2]);}
      } else {
        let middleTop, middleBot, toObj = getLine(doc, sel.to.line);
        if (sel.from.ch)
  ...
  function startWorker(cm, time) {
    if (cm.view.frontier < cm.display.showingTo) {cm.view.highlight.set(time, bind(highlightWorker, cm));}
  }

  function highlightWorker(cm) {
    let view = cm.view, doc = view.doc;
    if (view.frontier >= cm.display.showingTo) return;
    const end = +new Date + cm.options.workTime;
    const state = copyState(view.mode, getStateBefore(cm, view.frontier));
    let changed = [], prevChange;
    doc.iter(view.frontier, Math.min(doc.size, cm.display.showingTo + 500), (line) => {
    ...
    if (changed.length) {
      operation(cm, function () {
        for (let i = 0; i < changed.length; ++i) {regChange(this, changed[i].start, changed[i].end);}
      })();
    }
  }
  ...
  function findStartLine(cm, n) {
    let minindent, minline, doc = cm.view.doc;
    for (let search = n, lim = n - 100; search > lim; --search) {
  ...
  function getStateBefore(cm, n) {
    const view = cm.view;
    let pos = findStartLine(cm, n), state = pos && getLine(view.doc, pos - 1).stateAfter;
    if (!state) state = startState(view.mode);
    else state = copyState(view.mode, state);
    view.doc.iter(pos, n, (line) => {
      processLine(cm, line, state);
      const save = pos == n - 1 || pos % 5 == 0 || pos >= view.showingFrom && pos < view.showingTo;
  ...
  function paddingLeft(display) {
    const e = removeChildrenAndAdd(display.measure, elt('pre')).appendChild(elt('span', 'x'));
    return e.offsetLeft;
  }
  ...
  function measureLine(cm, line) {
    ...
    let display = cm.display, cache = cm.display.measureLineCache;
    for (let i = 0; i < cache.length; ++i) {
      var memo = cache[i];
      if (memo.text == line.text && memo.markedSpans == line.markedSpans &&
          display.scroller.clientWidth == memo.width) {return memo.measure;}
    }

    const measure = measureLineInner(cm, line);
    // Store result in the cache
    var memo = { text: line.text, width: display.scroller.clientWidth,
      markedSpans: line.markedSpans, measure };
  ...
  function measureLineInner(cm, line) {
    let display = cm.display, measure = emptyArray(line.text.length);
    const pre = lineContent(cm, line, measure);
    ...
    if (ie && !ie_lt8 && !cm.options.lineWrapping && pre.childNodes.length > 100) {
      const fragment = document.createDocumentFragment();
      let chunk = 10, n = pre.childNodes.length;
      for (var i = 0, chunks = Math.ceil(n / chunk); i < chunks; ++i) {
        const wrap = elt('div', null, null, 'display: inline-block');
    ...
    const outer = display.lineDiv.getBoundingClientRect();
    let vranges = [], data = emptyArray(line.text.length), maxBot = pre.offsetHeight;
    for (var i = 0, cur; i < measure.length; ++i) {
      if (cur = measure[i]) {
        const size = cur.getBoundingClientRect();
        let top = Math.max(0, size.top - outer.top), bot = Math.min(size.bottom - outer.top, maxBot);
        for (var j = 0; j < vranges.length; j += 2) {
          let rtop = vranges[j], rbot = vranges[j + 1];
  ...
  function intoCoordSystem(cm, lineObj, rect, context) {
    if (lineObj.widgets) {
      for (let i = 0; i < lineObj.widgets.length; ++i) {
        if (lineObj.widgets[i].above) {
          const size = lineObj.widgets[i].node.offsetHeight;
    ...
    let yOff = heightAtLine(cm, lineObj);
    if (context != 'local') yOff -= cm.display.viewOffset;
    if (context == 'page') {
      const lOff = cm.display.lineSpace.getBoundingClientRect();
      yOff += lOff.top + (window.pageYOffset || (document.documentElement || document.body).scrollTop);
      const xOff = lOff.left + (window.pageXOffset || (document.documentElement || document.body).scrollLeft);
  ...
  function cursorCoords(cm, pos, context, lineObj, measurement) {
    ...
    function get(ch, right) {
      const m = measureChar(cm, lineObj, ch, measurement);
      if (right) m.left = m.right; else m.right = m.left;
      return intoCoordSystem(cm, lineObj, m, context);
    }
    let order = getOrder(lineObj), ch = pos.ch;
    if (!order) return get(ch);
    let main, other, linedir = order[0].level;
    for (let i = 0; i < order.length; ++i) {
      var part = order[i], rtl = part.level % 2, nb, here;
      if (part.from < ch && part.to > ch) return get(ch, rtl);
      let left = rtl ? part.to : part.from, right = rtl ? part.from : part.to;
      if (left == ch) {
  ...
  function coordsChar(cm, x, y) {
    ...
    let lineNo = lineAtHeight(doc, y);
    if (lineNo >= doc.size) return { line: doc.size - 1, ch: getLine(doc, doc.size - 1).text.length };
    if (x < 0) x = 0;

    for (;;) {
      const lineObj = getLine(doc, lineNo);
      const found = coordsCharInner(cm, lineObj, lineNo, x, y);
      const merged = collapsedSpanAtEnd(lineObj);
      if (merged && found.ch == lineRight(lineObj)) {lineNo = merged.find().to.line;} else {return found;}
    }
  }

  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    const innerOff = y - heightAtLine(cm, lineObj);
    let wrongLine = false, cWidth = cm.display.wrapper.clientWidth;
    const measurement = measureLine(cm, lineObj);

    function getX(ch) {
      const sp = cursorCoords(cm, { line: lineNo, ch }, 'line',
        lineObj, measurement);
      wrongLine = true;
    ...
    let fromX = paddingLeft(cm.display), toX = getX(to);

    if (x > toX) return { line: lineNo, ch: to, outside: wrongLine };
    // Do a binary search between these bounds.
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        let after = x - fromX < toX - x, ch = after ? from : to;
        while (isExtendingChar.test(lineObj.text.charAt(ch))) ++ch;
        return { line: lineNo, ch, after, outside: wrongLine };
      }
      ...
      if (middleX > x) {to = middle; toX = middleX; if (wrongLine) toX += 1000; dist -= step;} else {from = middle; fromX = middleX; dist = step;}
    }
  }
  ...
  function endOperation(cm) {
    ...
    for (let i = 0; i < op.delayedCallbacks.length; ++i) op.delayedCallbacks[i](cm);
  }
  ...
  function operation(cm1, f) {
    return function () {
      const cm = cm1 || this;
      startOperation(cm);
      try {var result = f.apply(cm, arguments);} finally {endOperation(cm);}
      return result;
    };
  }

  function regChange(cm, from, to, lendiff) {
    cm.curOp.changes.push({ from, to, diff: lendiff });
  }
  ...
  function readInput(cm) {
    let input = cm.display.input, prevInput = cm.display.prevInput, view = cm.view, sel = view.sel;
    ...
    if (same < prevInput.length) {from = { line: from.line, ch: from.ch - (prevInput.length - same) };} else if (view.overwrite && posEq(from, to) && !cm.display.pasteIncoming) {to = { line: to.line, ch: Math.min(getLine(cm.view.doc, to.line).text.length, to.ch + (text.length - same)) };}
    const updateInput = cm.curOp.updateInput;
    updateDoc(cm, from, to, splitLines(text.slice(same)), 'end',
      cm.display.pasteIncoming ? 'paste' : 'input', { from, to });
  ...
  function resetInput(cm, user) {
    let view = cm.view, minimal, selected;
  ...
  function registerEventHandlers(cm) {
    ...
    if (!gecko) on(d.scroller, 'contextmenu', (e) => {onContextMenu(cm, e);});
    ...
    on(d.input, 'keyup', operation(cm, (e) => {
    ...
    on(d.scroller, 'paste', () => {focusInput(cm); fastPoll(cm);});
    on(d.input, 'paste', () => {
  ...
  function onMouseDown(e) {
    let cm = this, display = cm.display, view = cm.view, sel = view.sel, doc = view.doc;
    ...
    if (lastDoubleClick && lastDoubleClick.time > now - 400 && posEq(lastDoubleClick.pos, start)) {
      ...
      e_preventDefault(e);
      const word = findWordAt(getLine(doc, start.line).text, start);
    ...
    let startstart = sel.from, startend = sel.to;
    ...
    function doSelect(cur) {
      ...
      if (type == 'double') {
        const word = findWordAt(getLine(doc, cur.line).text, cur);
    ...
    function extend(e) {
      ...
      const cur = posFromMouse(cm, e, true);
      if (!cur) return;
      if (!posEq(cur, last)) {
        if (!view.focused) onFocus(cm);
        last = cur;
  ...
  function onDrop(e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      const loadFile = function (file, i) {
        ...
        reader.onload = function () {
          ...
          if (++read == n) {
            ...
            operation(cm, () => {
              const end = replaceRange(cm, text.join(''), pos, pos, 'paste');
      ...
      try {
        ...
        if (text) {
          let curFrom = cm.view.sel.from, curTo = cm.view.sel.to;
  ...
  function clickInGutter(cm, e) {
    ...
    for (let i = 0; i < cm.options.gutters.length; ++i) {
      ...
      if (g && g.getBoundingClientRect().right >= mX) {
        const line = lineAtHeight(cm.view.doc, mY);
  ...
  let wheelSamples = 0, wheelDX, wheelDY, wheelStartX, wheelStartY, wheelPixelsPerUnit = null;
  ...
  function onScrollWheel(cm, e) {
    ...
    if (dy && mac && webkit) {
      for (let cur = e.target; cur != scroll; cur = cur.parentNode) {
        if (cur.lineObj) {
    ...
    if (dy && wheelPixelsPerUnit != null) {
      ...
      let top = cm.view.scrollTop, bot = top + cm.display.wrapper.clientHeight;
      if (pixels < 0) top = Math.max(0, top + pixels - 50);
      else bot = Math.min(cm.view.doc.height, bot + pixels + 50);
      updateDisplay(cm, [], { top, bottom: bot });
    }
    ...
    if (wheelSamples < 20) {
      if (wheelStartX == null) {
        ...
        setTimeout(() => {
          ...
          const movedX = scroll.scrollLeft - wheelStartX;
          const movedY = scroll.scrollTop - wheelStartY;
          const sample = (movedY && wheelDY && movedY / wheelDY) ||
  ...
  function doHandleBinding(cm, bound, dropShift) {
    ...
    let view = cm.view, prevShift = view.sel.shift;
    try {
  ...
  function allKeyMaps(cm) {
    const maps = cm.view.keyMaps.slice(0);
  ...
  function handleKeyBinding(cm, e) {
    ...
    let startMap = getKeyMap(cm.options.keyMap), next = startMap.auto;
    ...
    let name = keyNames[e_prop(e, 'keyCode')], handled = false;
    const flipCtrlCmd = mac && (opera || qtwebkit);
  ...
  function onKeyPress(e) {
    const cm = this;
    if (cm.options.onKeyEvent && cm.options.onKeyEvent(cm, addStop(e))) return;
    let keyCode = e_prop(e, 'keyCode'), charCode = e_prop(e, 'charCode');
    ...
    if (this.options.electricChars && this.view.mode.electricChars &&
        ...
        this.view.mode.electricChars.indexOf(ch) > -1) {setTimeout(operation(cm, () => {indentLine(cm, cm.view.sel.to.line, 'smart');}), 75);}
  ...
  function onBlur(cm) {
    ...
    setTimeout(() => {if (!cm.view.focused) cm.view.sel.shift = false;}, 150);
  }

  let detectingSelectAll;
  function onContextMenu(cm, e) {
    let display = cm.display, sel = cm.view.sel;
    ...
    function rehide() {
      ...
      if (display.input.selectionStart != null) {
        ...
        let extval = display.input.value = ` ${ posEq(sel.from, sel.to) ? '' : display.input.value}`, i = 0;
        display.prevInput = ' ';
  ...
  function updateDoc(cm, from, to, newText, selUpdate, origin) {
    ...
    const split = sawReadOnlySpans &&
      removeReadOnlyRanges(cm.view.doc, from, to);
    if (split) {
      for (let i = split.length - 1; i >= 1; --i) {updateDocInner(cm, split[i].from, split[i].to, [''], origin);}
  ...
  function updateDocInner(cm, from, to, newText, selUpdate, origin) {
    ...
    let view = cm.view, doc = view.doc, old = [];
    doc.iter(from.line, to.line + 1, (line) => {
      old.push(newHL(line.text, line.markedSpans));
    });
    let startSelFrom = view.sel.from, startSelTo = view.sel.to;
    const lines = updateMarkedSpans(hlSpans(old[0]), hlSpans(lst(old)), from.ch, to.ch, newText);
    const retval = updateDocNoUndo(cm, from, to, lines, selUpdate, origin);
    if (view.history) {
  ...
  function unredoHelper(cm, type) {
    let doc = cm.view.doc, hist = cm.view.history;
    const set = (type == 'undo' ? hist.done : hist.undone).pop();
    if (!set) return;
    const anti = { events: [], fromBefore: set.fromAfter, toBefore: set.toAfter,
      fromAfter: set.fromBefore, toAfter: set.toBefore };
    for (let i = set.events.length - 1; i >= 0; i -= 1) {
      hist.dirtyCounter += type == 'undo' ? -1 : 1;
      const change = set.events[i];
      ...
      const selPos = i ? null : { from: set.fromBefore, to: set.toBefore };
  ...
  function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
    let view = cm.view, doc = view.doc, display = cm.display;
    if (view.suppressEdits) return;

    let nlines = to.line - from.line, firstLine = getLine(doc, from.line), lastLine = getLine(doc, to.line);
    ...
    let lastHL = lst(lines), th = textHeight(display);
    ...
    if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == '') {
      ...
      var added = [];
      for (var i = 0, e = lines.length - 1; i < e; ++i) {added.push(makeLine(hlText(lines[i]), hlSpans(lines[i]), th));}
      ...
      if (lines.length == 1) {
        ...
        for (var added = [], i = 1, e = lines.length - 1; i < e; ++i) {added.push(makeLine(hlText(lines[i]), hlSpans(lines[i]), th));}
      ...
      for (var i = 1, e = lines.length - 1; i < e; ++i) {added.push(makeLine(hlText(lines[i]), hlSpans(lines[i]), th));}
      if (nlines > 1) doc.remove(from.line + 1, nlines - 1, cm);
      doc.insert(from.line + 1, added);
    }

    if (cm.options.lineWrapping) {
      const perLine = Math.max(5, display.scroller.clientWidth / charWidth(display) - 3);
      doc.iter(from.line, from.line + lines.length, (line) => {
        if (line.height == 0) return;
        const guess = (Math.ceil(line.text.length / perLine) || 1) * th;
        if (guess != line.height) updateLineHeight(line, guess);
      });
    } else {
      doc.iter(checkWidthStart, from.line + lines.length, (line) => {
        const len = lineLength(doc, line);
        if (len > view.maxLineLength) {
          view.maxLine = line;
          view.maxLineLength = len;
          view.maxLineChanged = true;
    ...
    if (hasHandler(cm, 'change')) {
      ...
      const changeObj = { from, to, text: lines, origin };
    ...
    let newSelFrom, newSelTo, end = { line: from.line + lines.length - 1,
      ch: hlText(lastHL).length + (lines.length == 1 ? from.ch : 0) };
    if (selUpdate && typeof selUpdate !== 'string') {
      if (selUpdate.from) { newSelFrom = selUpdate.from; newSelTo = selUpdate.to; } else newSelFrom = newSelTo = selUpdate;
      ...
      const adjustPos = function (pos) {
        if (posLess(pos, from)) return pos;
        if (!posLess(to, pos)) return end;
        const line = pos.line + lendiff;
        let ch = pos.ch;
  ...
  function clipPos(doc, pos) {
    ...
    let ch = pos.ch, linelen = getLine(doc, pos.line).text.length;
  ...
  function extendSelection(cm, pos, other, bias) {
    const sel = cm.view.sel;
    if (sel.shift || sel.extend) {
      let anchor = sel.anchor;
      if (other) {
        const posBefore = posLess(pos, anchor);
  ...
  function skipAtomic(cm, pos, bias, mayClear) {
    let doc = cm.view.doc, flipped = false, curPos = pos;
    let dir = bias || 1;
      ...
      if (line.markedSpans) {
        for (var i = 0; i < line.markedSpans.length; ++i) {
          ...
          if ((sp.from == null || (m.inclusiveLeft ? sp.from <= curPos.ch : sp.from < curPos.ch)) &&
            ...
            let newPos = m.find()[dir < 0 ? 'from' : 'to'];
  ...
  function scrollPosIntoView(cm, pos) {
    for (;;) {
      let changed = false, coords = cursorCoords(cm, pos);
      const scrollPos = calculateScrollPos(cm, coords.left, coords.top, coords.left, coords.bottom);
      let startTop = cm.view.scrollTop, startLeft = cm.view.scrollLeft;
  ...
  function calculateScrollPos(cm, x1, y1, x2, y2) {
    let display = cm.display, pt = paddingTop(display);
    y1 += pt; y2 += pt;
    let screen = display.scroller.clientHeight - scrollerCutOff, screentop = display.scroller.scrollTop, result = {};
    const docBottom = cm.view.doc.height + 2 * pt;
    let atTop = y1 < pt + 10, atBottom = y2 + pt > docBottom - 10;
    if (y1 < screentop) result.scrollTop = atTop ? 0 : Math.max(0, y1);
    else if (y2 > screentop + screen) result.scrollTop = (atBottom ? docBottom : y2) - screen;

    let screenw = display.scroller.clientWidth - scrollerCutOff, screenleft = display.scroller.scrollLeft;
    x1 += display.gutters.offsetWidth; x2 += display.gutters.offsetWidth;
    const gutterw = display.gutters.offsetWidth;
    const atLeft = x1 < gutterw + 10;
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (indentString != curSpaceString) {replaceRange(cm, indentString, { line: n, ch: 0 }, { line: n, ch: curSpaceString.length }, 'input');}
    line.stateAfter = null;
  }

  function changeLine(cm, handle, op) {
    let no = handle, line = handle, doc = cm.view.doc;
  ...
  function findPosH(cm, dir, unit, visually) {
    let doc = cm.view.doc, end = cm.view.sel.head, line = end.line, ch = end.ch;
    ...
    return skipAtomic(cm, { line, ch }, dir, true);
  }
  ...
  function findWordAt(line, pos) {
    ...
    if (line) {
      ...
      const check = isWordChar(startChar) ? isWordChar :
  ...
  function selectLine(cm, line) {
    extendSelection(cm, { line, ch: 0 }, clipPos(cm.view.doc, { line: line + 1, ch: 0 }));
  }
  ...
  CodeMirror.prototype = {
    getValue(lineSep) {
      let text = [], doc = this.view.doc;
      doc.iter(0, doc.size, (line) => { text.push(line.text); });
      return text.join(lineSep || '\n');
    },

    setValue: operation(null, function (code) {
      let doc = this.view.doc, top = { line: 0, ch: 0 }, lastLen = getLine(doc, doc.size - 1).text.length;
      updateDocInner(this, top, { line: doc.size - 1, ch: lastLen }, splitLines(code), top, top, 'setValue');
    }),

    getSelection(lineSep) { return this.getRange(this.view.sel.from, this.view.sel.to, lineSep); },
    ...
    focus() {window.focus(); focusInput(this); onFocus(this); fastPoll(this);},
    ...
    indentSelection: operation(null, function (how) {
      ...
      for (let i = sel.from.line; i <= e; ++i) indentLine(this, i, how);
    }),
    ...
    clearHistory() {this.view.history = makeHistory();},
    ...
    isClean() {return this.view.history.dirtyCounter == 0;},
    ...
    getHistory() {
      ...
      function cp(arr) {
        for (var i = 0, nw = [], nwelt; i < arr.length; ++i) {
          ...
          for (let j = 0, elt = set.events; j < elt.length; ++j) {
            let old = [], cur = elt[j];
            nwelt.push({ start: cur.start, added: cur.added, old });
            for (let k = 0; k < cur.old.length; ++k) old.push(hlText(cur.old[k]));
          }
        }
        return nw;
      }
    ...
    setHistory(histData) {
      const hist = this.view.history = makeHistory();
    ...
    getTokenAt(pos) {
      ...
      pos = clipPos(doc, pos);
      let state = getStateBefore(this, pos.line), mode = this.view.mode;
    ...
    coordsChar(coords) {
      const off = this.display.lineSpace.getBoundingClientRect();
    ...
    findMarksAt(pos) {
      const doc = this.view.doc;
      pos = clipPos(doc, pos);
      let markers = [], spans = getLine(doc, pos.line).markedSpans;
      if (spans) {
        for (let i = 0; i < spans.length; ++i) {
          const span = spans[i];
          if ((span.from == null || span.from <= pos.ch) &&
            (span.to == null || span.to >= pos.ch)) {markers.push(span.marker);}
        }
      }
      return markers;
    },
    ...
    setGutterMarker: operation(null, function (line, gutterID, value) {
      return changeLine(this, line, (line) => {
    ...
    clearGutter: operation(null, function (gutterID) {
      let i = 0, cm = this, doc = cm.view.doc;
      doc.iter(0, doc.size, (line) => {
    ...
    addLineClass: operation(null, function (handle, where, cls) {
      return changeLine(this, handle, (line) => {
    ...
    removeLineClass: operation(null, function (handle, where, cls) {
      return changeLine(this, handle, (line) => {
        ...
        if (!cur) return false;
          ...
          const upd = cur.replace(new RegExp(`^${ cls }\\b\\s*|\\s*\\b${ cls }\\b`), '');
    ...
    removeLineWidget: operation(null, function (widget) {
      let ws = widget.line.widgets, no = lineNo(widget.line);
      if (no == null) return;
      for (let i = 0; i < ws.length; ++i) if (ws[i] == widget) ws.splice(i--, 1);
    ...
    getViewport() { return { from: this.display.showingFrom, to: this.display.showingTo };},
    ...
    addWidget(pos, node, scroll, vert, horiz) {
      ...
      if (vert == 'over') top = pos.top;
        ...
        let vspace = Math.max(display.wrapper.clientHeight, this.view.doc.height),
    ...
    lineCount() {return this.view.doc.size;},

    clipPos(pos) {return clipPos(this.view.doc, pos);},

    getCursor(start) {
      let sel = this.view.sel, pos;
    ...
    somethingSelected() {return !posEq(this.view.sel.from, this.view.sel.to);},

    setCursor: operation(null, function (line, ch, extend) {
      const pos = clipPos(this.view.doc, typeof line === 'number' ? { line, ch: ch || 0 } : line);
    ...
    setExtending(val) {this.view.sel.extend = val;},
    ...
    setLine: operation(null, function (line, text) {
      if (isLine(this.view.doc, line)) {replaceRange(this, text, { line, ch: 0 }, { line, ch: getLine(this.view.doc, line).text.length });}
    }),

    removeLine: operation(null, function (line) {
      if (isLine(this.view.doc, line)) {replaceRange(this, '', { line, ch: 0 }, clipPos(this.view.doc, { line: line + 1, ch: 0 }));}
    }),
    ...
    getRange(from, to, lineSep) {
      ...
      const code = [getLine(doc, l1).text.slice(from.ch)];
    ...
    execCommand(cmd) {return commands[cmd](this);},

    // Stuff used by commands, probably not much use to outside code.
    moveH: operation(null, function (dir, unit) {
      let sel = this.view.sel, pos = dir < 0 ? sel.from : sel.to;
    ...
    moveV: operation(null, function (dir, unit) {
      let view = this.view, doc = view.doc, display = this.display;
      let cur = view.sel.head, pos = cursorCoords(this, cur, 'div');
      let x = pos.left, y;
      if (view.goalColumn != null) x = view.goalColumn;
      if (unit == 'page') {
        const pageSize = Math.min(display.wrapper.clientHeight, window.innerHeight || document.documentElement.clientHeight);
    ...
    toggleOverwrite() {
      if (this.view.overwrite = !this.view.overwrite) {this.display.cursor.className += ' CodeMirror-overwrite';} else {this.display.cursor.className = this.display.cursor.className.replace(' CodeMirror-overwrite', '');}
    },

    posFromIndex(off) {
      let lineNo = 0, ch, doc = this.view.doc;
      doc.iter(0, doc.size, (line) => {
      ...
      return clipPos(doc, { line: lineNo, ch });
    },
    indexFromPos(coords) {
      if (coords.line < 0 || coords.ch < 0) return 0;
      let index = coords.ch;
      this.view.doc.iter(0, coords.line, (line) => {
    ...
    getScrollInfo() {
      let scroller = this.display.scroller, co = scrollerCutOff;
    ...
    operation(f) {return operation(this, f)();},

    refresh() {
      clearCaches(this);
      if (this.display.scroller.scrollHeight > this.view.scrollTop) {this.display.scrollbarV.scrollTop = this.display.scroller.scrollTop = this.view.scrollTop;}
  ...
  CodeMirror.defineMode = function (name, mode) {
    ...
    if (arguments.length > 2) {
      mode.dependencies = [];
      for (let i = 2; i < arguments.length; ++i) mode.dependencies.push(arguments[i]);
    }
    modes[name] = mode;
  };
  ...
  var commands = CodeMirror.commands = {
    ...
    killLine(cm) {
      ...
      if (!sel && cm.getLine(from.line).length == from.ch) {cm.replaceRange('', from, { line: from.line + 1, ch: 0 }, 'delete');} else cm.replaceRange('', from, sel ? to : { line: from.line }, 'delete');
    },
    deleteLine(cm) {
    ...
    undo(cm) {cm.undo();},
    redo(cm) {cm.redo();},
    goDocStart(cm) {cm.extendSelection({ line: 0, ch: 0 });},
    goDocEnd(cm) {cm.extendSelection({ line: cm.lineCount() - 1 });},
    goLineStart(cm) {
      cm.extendSelection(lineStart(cm, cm.getCursor().line));
    },
    goLineStartSmart(cm) {
      let cur = cm.getCursor(), start = lineStart(cm, cur.line);
      const line = cm.getLineHandle(start.line);
      const order = getOrder(line);
      if (!order || order[0].level == 0) {
        const firstNonWS = Math.max(0, line.text.search(/\S/));
        const inWS = cur.line == start.line && cur.ch <= firstNonWS && cur.ch;
    ...
    insertTab(cm) {cm.replaceSelection('\t', 'end', 'input');},
    defaultTab(cm) {
  ...
  function lookupKey(name, maps, handle, stop) {
    ...
    for (let i = 0; i < maps.length; ++i) {if (lookup(maps[i])) return true;}
  }
  ...
  TextMarker.prototype.clear = function () {
    ...
    for (let i = 0; i < this.lines.length; ++i) {
      ...
      if (span.from != null) {min = lineNo(line);} else if (this.collapsed && !lineIsHidden(line)) {updateLineHeight(line, textHeight(this.cm.display));}
    }
  ...
  function markText(cm, from, to, options, type) {
    const doc = cm.view.doc;
    const marker = new TextMarker(cm, type);
    ...
    let curLine = from.line, size = 0, collapsedAtStart, collapsedAtEnd;
    doc.iter(curLine, to.line + 1, (line) => {
      const span = { from: null, to: null, marker };
  ...
  function updateMarkedSpans(oldFirst, oldLast, startCh, endCh, newText) {
    ...
    const last = markedSpansAfter(oldLast, startCh, endCh);

    // Next, merge those two ends
    let sameLine = newText.length == 1, offset = lst(newText).length + (sameLine ? startCh : 0);
    if (first) {
    ...
    if (!sameLine) {
      ...
      let gap = newText.length - 2, gapMarkers;
      if (gap > 0 && first) {
        for (var i = 0; i < first.length; ++i) {
  ...
  function hlSpans(val) {
    ...
    let spans = val.markedSpans, out = null;
    for (let i = 0; i < spans.length; ++i) {
  ...
  function attachMarkedSpans(line, spans) {
    if (!spans) return;
    for (let i = 0; i < spans.length; ++i) {spans[i].marker.lines.push(line);}
  ...
  function highlightLine(cm, line, state) {
    let mode = cm.view.mode, flattenSpans = cm.options.flattenSpans;
    let changed = !line.styles, pos = 0, curText = '', curStyle = null;
    let stream = new StringStream(line.text, cm.options.tabSize), st = line.styles || (line.styles = []);
    if (line.text == '' && mode.blankLine) mode.blankLine(state);
    while (!stream.eol()) {
      let style = mode.token(stream, state), substr = stream.current();
      stream.start = stream.pos;
  ...
  function processLine(cm, line, state) {
    const mode = cm.view.mode;
    const stream = new StringStream(line.text, cm.options.tabSize);
    if (line.text == '' && mode.blankLine) mode.blankLine(state);
  ...
  function lineContent(cm, realLine, measure) {
    let merged, line = realLine, lineBefore, sawBefore, simple = true;
    ...
    const builder = { pre: elt('pre'), col: 0, pos: 0, display: !measure,
      measure: null, addedOne: false, cm };
    if (line.textClass) builder.pre.className = line.textClass;

    do {
      if (!line.styles) {highlightLine(cm, line, line.stateAfter = getStateBefore(cm, lineNo(line)));}
  ...
  function buildToken(builder, text, style, startStyle, endStyle) {
    ...
    if (!tokenSpecialChars.test(text)) {
      ...
      while (true) {
        ...
        const m = tokenSpecialChars.exec(text);
  ...
  function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
    for (let i = 0; i < text.length; ++i) {
      if (i && i < text.length - 1 &&
          ...
          spanAffectsWrapping.test(text.slice(i - 1, i + 1))) {builder.pre.appendChild(elt('wbr'));}
  ...
  function insertLineContent(line, builder) {
    let st = line.styles, spans = line.markedSpans;
    if (!spans) {
      for (var i = 0; i < st.length; i += 2) {builder.addToken(builder, st[i], styleToClass(st[i + 1]));}
      return;
    }

    let allText = line.text, len = allText.length;
    var pos = 0, i = 0, text = '', style;
    let nextChange = 0, spanStyle, spanEndStyle, spanStartStyle, collapsed;
    for (;;) {
  ...
  function addChange(cm, start, added, old, origin, fromBefore, toBefore, fromAfter, toAfter) {
    const history = cm.view.history;
    history.undone.length = 0;
    let time = +new Date, cur = lst(history.done);
    ...
    if (cur &&
      ...
      {history.dirtyCounter = NaN;} else {history.dirtyCounter++;}
    }
  ...
  function off(emitter, type, f) {
    if (emitter.removeEventListener) {emitter.removeEventListener(type, f, false);} else if (emitter.detachEvent) {emitter.detachEvent(`on${ type}`, f);} else {
      ...
      if (!arr) return;
      for (let i = 0; i < arr.length; ++i) {if (arr[i] == f) { arr.splice(i, 1); break; }}
    }
  }
  ...
  function signalLater(cm, emitter, type /* , values...*/) {
    const arr = emitter._handlers && emitter._handlers[type];
    if (!arr) return;
    let args = Array.prototype.slice.call(arguments, 3), flist = cm.curOp && cm.curOp.delayedCallbacks;
  ...
  const nonASCIISingleCaseWordChar = /[\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc]/;
  ...
  function isEmpty(obj) {
    let c = 0;
    for (const n in obj) if (obj.hasOwnProperty(n) && obj[n]) ++c;
    return !c;
  }
  ...
  function iterateBidiSections(order, from, to, f) {
    ...
    for (let i = 0; i < order.length; ++i) {
      const part = order[i];
      if (part.from < to && part.to > from) {f(Math.max(part.from, from), Math.min(part.to, to), part.level == 1 ? 'rtl' : 'ltr');}
    }
  }
  ...
  function lineStart(cm, lineN) {
    const line = getLine(cm.view.doc, lineN);
    const visual = visualLine(cm.view.doc, line);
    if (visual != line) lineN = lineNo(visual);
    const order = getOrder(visual);
    const ch = !order ? 0 : order[0].level % 2 ? lineRight(visual) : lineLeft(visual);
    return { line: lineN, ch };
  }
  function lineEnd(cm, lineNo) {
    let merged, line;
    while (merged = collapsedSpanAtEnd(line = getLine(cm.view.doc, lineNo))) {lineNo = merged.find().to.line;}
    const order = getOrder(line);
    const ch = !order ? line.text.length : order[0].level % 2 ? lineLeft(line) : lineRight(line);
    return { line: lineNo, ch };
  }
  ...
  function moveVisually(line, start, dir, byUnit) {
    ...
    const moveOneUnit = byUnit ? function (pos, dir) {
      do pos += dir;
      while (pos > 0 && isExtendingChar.test(line.text.charAt(pos)));
      return pos;
    } : function (pos, dir) { return pos + dir; };
    const linedir = bidi[0].level;
    for (var i = 0; i < bidi.length; ++i) {
      var part = bidi[i], sticky = part.level % 2 == linedir;
      if ((part.from < start && part.to > start) ||
          (sticky && (part.from == start || part.to == start))) break;
    }
    let target = moveOneUnit(start, part.level % 2 ? -dir : dir);
  ...
  var bidiOrdering = (function () {
    ...
    const lowTypes = 'bbbbbbbbbtstwsbbbbbbbbbbbbbbssstwNN%%%NNNNNN,N,N1111111111NNNNNNNLLLLLLLLLLLLLLLLLLLLLLLLLLNNNNNNLLLLLLLLLLLLLLLLLLLLLLLLLLNNNNbbbbbbsbbbbbbbbbbbbbbbbbbbbbbbbbb,N%%%%NNNNLNNNNN%%11NLNNN1LNNNNNLLLLLLLLLLLLLLLLLLLLLLLNLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLNLLLLLLLL';
    // Character types for codepoints 0x600 to 0x6ff
    const arabicTypes = 'rrrrrrrrrrrr,rNNmmmmmmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmrrrrrrrnnnnnnnnnn%nnrrrmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmmmmmmNmmmmrrrrrrrrrrrrrrrrrr';
    ...
    return function charOrdering(str) {
      ...
      let len = str.length, types = [], startType = null;
      ...
      for (var i = 0; i < len; ++i) {
        if (isNeutral.test(types[i])) {
          ...
          const before = (i ? types[i - 1] : startType) == 'L';
          const after = (end < len - 1 ? types[end] : startType) == 'L';
