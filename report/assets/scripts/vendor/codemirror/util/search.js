...
(function () {
    ...
    dialog(cm, queryDialog, 'Search for:', (query) => {
      cm.operation(() => {
        ...
        if (cm.lineCount() < 2000) { // This is too expensive on big documents.
          for (let cursor = getSearchCursor(cm, state.query); cursor.findNext();) {
  ...
  const replacementQueryDialog = 'With: <input type="text" style="width: 10em"/>';
  const doReplaceConfirm = 'Replace? <button>Yes</button> <button>No</button> <button>Stop</button>';
