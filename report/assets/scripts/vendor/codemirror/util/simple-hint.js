(function () {
  CodeMirror.simpleHint = function (editor, getHints, givenOptions) {
    ...
    let options = {}, defaults = CodeMirror.simpleHint.defaults;
    ...
    function collectHints(previousToken) {
      ...
      const tempToken = editor.getTokenAt(editor.getCursor());
      ...
      const result = getHints(editor, givenOptions);
      if (!result || !result.list.length) return;
      const completions = result.list;
      function insert(str) {
      ...
      const sel = complete.appendChild(document.createElement('select'));
      ...
      const pos = editor.cursorCoords(options.alignWithWord ? result.from : null);
      ...
      if(winW - pos.left < sel.clientWidth) {complete.style.left = `${pos.left - sel.clientWidth }px`;}
      ...
      CodeMirror.on(sel, 'keydown', (event) => {
        const code = event.keyCode;
        // Enter
        if (code == 13) {CodeMirror.e_stop(event); pick();}
        // Escape
        else if (code == 27) {CodeMirror.e_stop(event); close(); editor.focus();} else if (code != 38 && code != 40 && code != 33 && code != 34 && !CodeMirror.isModifierKey(event)) {
          close(); editor.focus();
