(function () {
  function SearchCursor(cm, query, pos, caseFold) {
    ...
    if (typeof query !== 'string') { // Regexp match
      ...
      if (target.length == 1) {
        this.matches = function (reverse, pos) {
          let line = fold(cm.getLine(pos.line)), len = query.length, match;
        ...
        this.matches = function (reverse, pos) {
          let ln = pos.line, idx = (reverse ? target.length - 1 : 0), match = target[idx], line = fold(cm.getLine(ln));
          ...
          for (;;) {
            ...
            let start = { line: pos.line, ch: offsetA }, end = { line: ln, ch: offsetB };
  ...
  SearchCursor.prototype = {
    ...
    find(reverse) {
      let self = this, pos = this.cm.clipPos(reverse ? this.pos.from : this.pos.to);
