...
(function () {
  CodeMirror.defineOption('autoCloseTags', false, (cm, val, old) => {
    if (val && (old == CodeMirror.Init || !old)) {
      ...
      if (typeof val !== 'object' || val.whenClosing) {map["'/'"] = function (cm) { autoCloseTag(cm, '/'); };}
      if (typeof val !== 'object' || val.whenOpening) {map["'>'"] = function (cm) { autoCloseTag(cm, '>'); };}
      cm.addKeyMap(map);
  ...
  function autoCloseTag(cm, ch) {
    ...
    if (ch == '>' && state.tagName) {
      ...
      if (tok.type == 'tag' && state.type == 'closeTag' ||
          /\/\s*$/.test(tok.string) ||
          dontCloseTags && indexOf(dontCloseTags, lowerTagName) > -1) {throw CodeMirror.Pass;}

      const doIndent = indentTags && indexOf(indentTags, lowerTagName) > -1;
